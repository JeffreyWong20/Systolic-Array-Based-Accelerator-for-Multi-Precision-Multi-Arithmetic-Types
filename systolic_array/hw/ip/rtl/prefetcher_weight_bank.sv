...
module prefetcher_weight_bank #(
    ...
    output logic [AXI_ADDRESS_WIDTH-1:0]                      weight_bank_axi_rm_fetch_start_address, // the start address of each row of the weight matrix (multiple of 64 bytes)
    output logic [$clog2(MAX_FETCH_REQ_BYTE_COUNT)-1:0]       weight_bank_axi_rm_fetch_byte_count, // 4 * total number of features per row (each feature is 4 bytes)
...
always_ff @(posedge core_clk or negedge resetn) begin
    ...
    // Manage to send the request to the weight read master and now waiting for response for expected number of beats
    // This only happens when the weight state is at WEIGHT_BANK_FSM_FETCH_REQ and the weight read master is ready to accept the request (should happens once)
