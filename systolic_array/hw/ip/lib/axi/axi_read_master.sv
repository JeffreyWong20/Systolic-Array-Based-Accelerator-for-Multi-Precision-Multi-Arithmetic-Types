...
module axi_read_master #(
    ...
    output logic [AXI_ADDRESS_WIDTH-1:0]               axi_araddr,  // Address of the first beat of the read transaction to the slave
    output logic [1:0]                                 axi_arburst, // Burst type of the read transaction to the slave
    output logic [3:0]                                 axi_arcache, // Not used
    output logic [3:0]                                 axi_arid,    // Not used. ID of the read transacaption to the slave 
    output logic [7:0]                                 axi_arlen,   // Number of beats within a read transaction to the slave
    output logic [0:0]                                 axi_arlock,  // Not used
    output logic [2:0]                                 axi_arprot,  // Not used
    output logic [3:0]                                 axi_arqos,   // Not used
    output logic [2:0]                                 axi_arsize,  // Size of each beat within a read transaction to the slave
    ...
    output logic                                       axi_rready,  // Indicates that the master is ready to accept the data read from the slave
    input  logic [1:0]                                 axi_rresp    // Response from the slave
);

// (WIDTH_X - 1)/WIDTH_Y + 1 = WIDTH_X/WIDTH_Y (rounded up)
parameter MAX_TOTAL_BEATS = (MAX_BYTE_COUNT-1)/64 + 1; // 64
parameter MAX_TRANSACTIONS = (MAX_TOTAL_BEATS-1)/256 + 1; // 1  // 256 beats per transaction

typedef enum logic [2:0] {
    IDLE    = 3'd0, // Idle
    AR      = 3'd1, // Sending Read Request
    R       = 3'd2, // Receiving Read Response
    RESP    = 3'd3  // Sending Response
} fetch_fsm_e;
...
logic                                       accepting_fetch_request;        // fetch_req_valid && fetch_req_ready
logic                                       accepting_axi_read_transaction; // axi_arvalid && axi_arready
logic                                       accepting_axi_read_response;    // axi_rvalid && axi_rready

logic [$clog2(MAX_TOTAL_BEATS)-1:0]         required_beat_count;            // req_fetch_byte_count / 64
logic [7:0]                                 burst_length_final_transaction; // req_fetch_byte_count % 64
logic [$clog2(MAX_TRANSACTIONS)-1:0]        required_transaction_count;     // required_beat_count / 256

logic [$clog2(MAX_TRANSACTIONS)-1:0]        sent_transactions;              // number of transactions sent
logic [$clog2(MAX_TOTAL_BEATS)-1:0]         beats_requested;                // number of beats requested
logic [$clog2(MAX_TOTAL_BEATS)-1:0]         beats_received;                 // number of beats received
logic [7:0]                                 received_read_responses;        // number of read responses received
...
always_comb begin
    ...
    // number of beats required for the fetch request. 64 bytes per beat
    ...
    // the burst length and the number of transactions required for the fetch request
