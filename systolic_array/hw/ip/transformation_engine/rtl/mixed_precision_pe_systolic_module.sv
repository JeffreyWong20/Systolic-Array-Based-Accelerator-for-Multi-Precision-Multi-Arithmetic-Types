...
module mixed_precision_pe_systolic_module #(
...
logic [(HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1)*MATRIX_N-1:0] [HIGH_PRECISION_DATA_WIDTH-1:0]                 sys_bias_per_row_high;
...
logic [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT:0] [MATRIX_N:0] [MATRIX_N-1:0] [HIGH_PRECISION_ACCUMULATOR_WIDTH-1:0]  sys_module_pe_acc_high; // NOTE: Data will get writeback in a high precision format. Low precision will get padded back to high precision by using 0.
...
// ==================================================================================================================================================
logic [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT:0] [MATRIX_N-1:0] [MATRIX_N-1:0] [HIGH_PRECISION_DATA_WIDTH-1:0] debug_update_counter_high;
logic [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT:0] [MATRIX_N-1:0] [MATRIX_N-1:0] [HIGH_PRECISION_DATA_WIDTH-1:0] debug_update_counter_inv_high;
...
for (genvar sys_module = 0; sys_module < LOW_PRECISION_SYSTOLIC_MODULE_COUNT; sys_module++) begin
    // Driving from weight channel
    always_comb begin
        sys_module_down_in_low_valid [sys_module*MATRIX_N + (MATRIX_N-1) : sys_module*MATRIX_N] = {MATRIX_N{weight_channel_resp_valid}} & weight_channel_resp.valid_mask[(HIGH_PRECISION_SYSTOLIC_MODULE_COUNT + 1 + sys_module)*MATRIX_N + (MATRIX_N-1) : (HIGH_PRECISION_SYSTOLIC_MODULE_COUNT + 1 + sys_module)*MATRIX_N];
    end

    always_ff @(posedge core_clk or negedge resetn) begin
        if (!resetn | clean_sys_module_active) begin
            sys_module_active_m [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT + 1 + sys_module] <= '0;
        end else begin
            sys_module_active_m [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT + 1 + sys_module] <= |sys_module_down_in_low_valid[sys_module*MATRIX_N + (MATRIX_N-1) : sys_module*MATRIX_N] | sys_module_active_m [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1+sys_module];
        end
    end
    ...
    systolic_module #(
        ...
        .diagonal_flush_done                 (sys_module_flush_done    [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1+sys_module]),
    ...
    fixed_cast_single #(
        ...
        .data_out   (sys_module_pe_acc_casted[sys_module+HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1][0])
    );
    fixed_cast_single #(
        .IN_WIDTH (LOW_PRECISION_ACCUMULATOR_WIDTH),
        .IN_FRAC_WIDTH (2*LOW_PRECISION_FRACTIONAL_BITS),
        ...
        .data_out   (sys_module_pe_acc_casted[sys_module+HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1][1])
    );
    fixed_cast_single #(
        .IN_WIDTH (LOW_PRECISION_ACCUMULATOR_WIDTH),
        .IN_FRAC_WIDTH (2*LOW_PRECISION_FRACTIONAL_BITS),
        ...
        .data_out   (sys_module_pe_acc_casted[sys_module+HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1][2])
    );
    fixed_cast_single #(
        .IN_WIDTH (LOW_PRECISION_ACCUMULATOR_WIDTH),
        .IN_FRAC_WIDTH (2*LOW_PRECISION_FRACTIONAL_BITS),
        ...
        .data_out   (sys_module_pe_acc_casted[sys_module+HIGH_PRECISION_SYSTOLIC_MODULE_COUNT+1][3])
    );
end
...
always_comb begin
    sys_module_down_in_high_valid [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N] = {MATRIX_N{weight_channel_resp_valid}} & weight_channel_resp.valid_mask[HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N];
end

always_ff @(posedge core_clk or negedge resetn) begin
    if (!resetn | clean_sys_module_active) begin
        // sys_module_active_m [sys_module+1 : sys_module] <= '0;
        sys_module_active_m [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT] <= '0;

    end else begin
        // sys_module_active_m [sys_module+1 : sys_module] <= |sys_module_down_in_high_valid[sys_module*MATRIX_N + (MATRIX_N-1) : sys_module*MATRIX_N] | sys_module_active_m [sys_module+1 : sys_module];
        sys_module_active_m [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT] <= |sys_module_down_in_high_valid[HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N] | sys_module_active_m [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT];
    end
end

for (genvar index = HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N; index < (HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) + 1); index++) begin
    always_comb begin
        sys_module_down_in_high       [index] = weight_channel_resp.data[index][HIGH_PRECISION_DATA_WIDTH-1:0];
        sys_bias_per_row_high         [index] = layer_config_bias_value[index][HIGH_PRECISION_DATA_WIDTH-1:0];
    end
...
mp_systolic_module #(
    ...
    .sys_module_forward_in_valid         (sys_module_forward_high_valid  [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT]),
    .sys_module_forward_in               (sys_module_forward_high        [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT]),
    .sys_module_forward_in_pass          (sys_module_forward_high        [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT]), // not used

    .sys_module_down_in_valid            (sys_module_down_in_high_valid  [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N]),
    .sys_module_down_in                  (sys_module_down_in_high        [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N]),

    .sys_module_forward_out_valid        (sys_module_forward_low_valid   [0]),
    .sys_module_forward_out              (sys_module_forward_low         [0]),
    .sys_module_forward_out_pass          (), // not used

    .sys_module_down_out_valid           (sys_module_down_out_high_valid  [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N]),
    .sys_module_down_out                 (sys_module_down_out_high         [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N]),
    
    .bias_valid                          (bias_valid),
    .bias                                (sys_bias_per_row_high           [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N + (MATRIX_N-1) : HIGH_PRECISION_SYSTOLIC_MODULE_COUNT*MATRIX_N]),
    ...
    .sys_module_pe_acc                   (sys_module_pe_acc_high   [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT]),

    .diagonal_flush_done                 (sys_module_flush_done    [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT]),

    .layer_config_leaky_relu_alpha_value ('1), // not used

    .debug_update_counter                (debug_update_counter_high     [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT])
);


assign    sys_module_pe_acc_casted [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][0] = sys_module_pe_acc_high[HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][0][0];
assign    sys_module_pe_acc_casted [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][1] = sys_module_pe_acc_high[HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][0][1];
assign    sys_module_pe_acc_casted [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][2] = sys_module_pe_acc_high[HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][0][2];
assign    sys_module_pe_acc_casted [HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][3] = sys_module_pe_acc_high[HIGH_PRECISION_SYSTOLIC_MODULE_COUNT][0][3];
